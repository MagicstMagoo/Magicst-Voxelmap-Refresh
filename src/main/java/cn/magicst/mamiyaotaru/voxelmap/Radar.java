 ...
 public class Radar implements IRadar {
   private IVoxelMap master = null; private class_310 game;
   ...
   private final Logger logger = LogManager.getLogger(); private static final Int2ObjectMap LEVEL_TO_ID; private static final Map TEXTURES;
   static {
       ...
       try {
         ...
         if (resource.isPresent()) {
           ...
           try { properties.load(is);
             ...
             if (is != null) is.close();  } catch (Throwable throwable) { if (is != null)
   ...
   public void renderMapMobs(class_4587 matrixStack, int x, int y) {
     ...
     for (Contact contact : this.contacts) {
       boolean inRange; RenderSystem.setShader(class_757::method_34542);
       contact.updateLocation();
               ...
               if (contact.type != EnumMobs.WITHER && contact.type != EnumMobs.WITHERINVULNERABLE) {
                 if (contact.type != EnumMobs.VEX && contact.type != EnumMobs.VEXCHARGING) {
                   if (contact.type == EnumMobs.PUFFERFISH || contact.type == EnumMobs.PUFFERFISHHALF || contact.type == EnumMobs.PUFFERFISHFULL) {
                     ...
                     switch (size) { case 0:
                         contact.type = EnumMobs.PUFFERFISH; break;
                       case 1: contact.type = EnumMobs.PUFFERFISHHALF; break;
                       case 2: contact.type = EnumMobs.PUFFERFISHFULL; break; }
