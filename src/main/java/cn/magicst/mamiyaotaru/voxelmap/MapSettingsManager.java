...
public class MapSettingsManager implements ISettingsManager {
        ...
        for (in = new BufferedReader(new InputStreamReader(new FileInputStream(this.settingsFile), StandardCharsets.UTF_8.newDecoder())); (sCurrentLine = in.readLine()) != null; class_304.method_1426()) {
          ...
          switch (curLine[0]) {
            ...
            case "Map Size":
              ...
              break;
            case "Zoom Key":
              bindKey(this.keyBindZoom, curLine[1]);
              break;
            case "Fullscreen Key":
              bindKey(this.keyBindFullscreen, curLine[1]);
              break;
            case "Menu Key":
              bindKey(this.keyBindMenu, curLine[1]);
              break;
            case "Waypoint Menu Key":
              bindKey(this.keyBindWaypointMenu, curLine[1]);
              break;
            case "Waypoint Key":
              bindKey(this.keyBindWaypoint, curLine[1]);
              break;
            case "Mob Key":
              bindKey(this.keyBindMobToggle, curLine[1]);
              break;
            case "In-game Waypoint Key":
              bindKey(this.keyBindWaypointToggle, curLine[1]);
              break;
          }
        }
  ...
  public void saveAll() {
    ...
    try {
      ...
      MessageUtils.chatInfo("Saving Settings " + var5.getLocalizedMessage());
    }
  }
