...
public class Map implements Runnable, IMap {
  ...
  private int[] lastLightmapValues = new int[] {
  ...
  public void newWorldName() {
    ...
    StringBuilder subworldNameBuilder = (new StringBuilder(")).append(I18nUtils.getString("worldmap.multiworld.newworld", new Object[0])).append(":").append(" ");
  ...
  private void mapCalc(boolean full) {
    ...
    if (!full) {
      ...
      for (imageY = 32 * multi - 1; imageY >= 0; ) {
        int imageX = (offsetX > 0) ? (32 * multi - offsetX) : 0;
        for (;; imageY--) {
          if (imageX < ((offsetX > 0) ? (32 * multi) : -offsetX)) {
            color24 = getPixelColor(true, true, true, true, nether, caves, (class_1937)this.world, multi, startX, startZ, imageX, imageY);
            this.mapImages[this.zoom].setRGB(imageX, imageY, color24);
            imageX++;
            continue;
      ...
      for (Object o : labels) {
        ...
        if (label.segmentSize > minimumSize) {
          ...
          if (this.options.oldNorth) {
            write(matrixStack, name, (left + 256) - z - (nameWidth / 2), top + x - 3.0F, 16777215);
            continue;
          }
