 ...
 import cn.magicst.mamiyaotaru.voxelmap.gui.IGuiWaypoints;
 ...
 public class CommandUtils {
   private static final int NEW_WAYPOINT_COMMAND_LENGTH = "/newWaypoint ".length(); private static final String NEW_WAYPOINT_COMMAND = "/newWaypoint ";
   ...
   public static boolean checkForWaypoints(class_2561 chat) {
     ...
     ArrayList<class_2585> textComponents = new ArrayList<>();
     int count = 0;
     ...
     for (String waypointString : waypointStrings) {
       ...
       chatStyle = chatStyle.method_10949(new class_2568(class_2568.class_5247.field_24342, hover));
   ...
   private static Waypoint createWaypointFromChat(String details) {
     ...
     try {
       ...
       for (int t = 0; t < pairs.length; t++) {
         ...
         if (splitIndex != -1) {
           ...
           if (key.equals("name")) {
             ...
             y = Integer.valueOf(Integer.parseInt(value));
       ...
       if (x != null && z != null) {
         if (dimensions.size() == 1 && ((DimensionContainer)dimensions.first()).type.method_31110() != 1.0D) {
         ...
         waypoint = new Waypoint(name, x.intValue(), z.intValue(), y.intValue(), enabled, red, green, blue, suffix, world, dimensions);
       } 
     ...
     if (newWaypoint != null) {
       for (Waypoint existingWaypoint : AbstractVoxelMap.getInstance().getWaypointManager().getWaypoints()) {
         if (newWaypoint.getX() == existingWaypoint.getX() && newWaypoint.getZ() == existingWaypoint.getZ()) {
           if (control) {
             class_310.method_1551().method_1507((class_437)new GuiAddWaypoint((IGuiWaypoints)null, (IVoxelMap)AbstractVoxelMap.getInstance(), existingWaypoint, true));
           } else {
       ...
       if (control) {
         class_310.method_1551().method_1507((class_437)new GuiAddWaypoint((IGuiWaypoints)null, (IVoxelMap)AbstractVoxelMap.getInstance(), newWaypoint, false));
       } else {
