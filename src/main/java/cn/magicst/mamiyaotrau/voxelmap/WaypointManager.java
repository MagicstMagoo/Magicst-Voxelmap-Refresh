...
public class WaypointManager
  ...
  public void onResourceManagerReload(class_3300 resourceManager) {
    final List<class_2960> images = new ArrayList<>();
  ...
  public void newWorld(class_1937 world) {
    if (world == null) {
      ...
      String mapName; this.game = class_310.method_1551();
    ...
    for (Waypoint pt : this.wayPts) {
      ...
      if (pt.name.startsWith("Previous Death")) {
        if (this.options.deathpoints == 2) {
          ...
          pt.name = "Previous Death " + num + 1; continue;
        } 
        toDel.add(pt);
      } 
    } 
  ...
  public synchronized void setSubworldHash(String hash) {
    ...
    if (this.currentSubWorldName.equals("")) {
      setSubWorldDescriptor(this.currentSubWorldHash);
    }
  }
  ...
  private void loadBackgroundMapImage() {
    ...
    try {
      ...
      path = path + "/" + path;
      InputStream is = this.game.method_1478().method_14486(new class_2960("voxelmap", "images/backgroundmaps/" + path + "/map.png")).method_14482();
      ...
      gfx.dispose();
      is = this.game.method_1478().method_14486(new class_2960("voxelmap", "images/backgroundmaps/" + path + "/map.txt")).method_14482();
